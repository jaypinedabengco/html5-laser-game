...
(function() {
  //variables
  var LaserGunContainer = function(
    laserBodyImageSrc,
    laserPointImageSrc,
    staticLaserTopImageSrc,
    isStaticLaser,
    laserContainerStage,
    laserLineStage,
  ...
  p.initialize = function(
    ...
    if (startY !== undefined) this.y = startY;
    ...
    this.setBounds(
      0,
      0,
      this.laserGunBitmap.getBounds().width,
      this.laserGunBitmap.getBounds().height
  ...
  LaserGunContainer.prototype.connectTo = function(laserGunContainer) {
    ...
    var p1 = new LaserPointerBitmap(
      me.laserPointImageSrc,
      me.getBounds().width,
      me.getBounds().height
    );
    var p2 = new LaserPointerBitmap(
      me.laserPointImageSrc,
      me.getBounds().width,
      me.getBounds().height
    );
  ...
  p.handlePressMove = function(evt) {
    ...
    // if success, then prevent movement..
    if (toolsContainer.classList.contains("success")) {
      return false;
    }
    ...
    if (evt.target.laserCollisionEngine.isAllTargetOn()) {
      ...
      toolsContainer.classList.add("success");
    } else {
      toolsContainer.classList.remove("success");
    }

    //end

    if (evt.target.isStaticLaser == false) {
      // if static, then don't move target item
      ...
      else if (evt.target.x < cw) evt.target.x = cw;

      if (evt.target.y > stage.canvas.height - ch)
        evt.target.y = stage.canvas.height - ch;
      else if (evt.target.y < ch) evt.target.y = ch;
    }
